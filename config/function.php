<?php
  ...
  function getUsernameFormCode($code){
    global $dbcon;
    $data = [$code];
    $sql = "SELECT user_username FROM ex_login WHERE user_code = ? AND user_status = '1'";
    $stmt = $dbcon->prepare($sql);
    $stmt->execute($data);
    $row = $stmt->fetch();
  ...
  function getSoftwareSelect(){
    global $dbcon;
    $sql = "SELECT * FROM ex_software WHERE sw_status = '1' ORDER BY sw_id ASC";
    $stmt = $dbcon->prepare($sql);
    $stmt->execute();
    return $stmt;
  }
 ...
 function getManageStatusSelect(){
    global $dbcon;
